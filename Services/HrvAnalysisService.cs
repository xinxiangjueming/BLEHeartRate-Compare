...
namespace HeartRateMonitor.Services
{
    ...
    public static class HrvAnalysisService
    {
        ...
        /// <summary>
        /// DFA α2 长期窗口上界（拍数）。
        /// 设为 32 使 α2 在 ~128 个 RR 间期（约 2 分钟）后即可计算。
        /// 经典文献使用 64，但那需要 ~4 分钟数据。
        /// </summary>
        private const int DFA_N_MAX_LONG = 32;

        /// <summary>
        /// 计算 DFA α1 所需的最少 RR 间期数量。
        /// 窗口 4-16，至少 4 个窗口，约需 64 个 RR 间期（约 1 分钟）。
        /// </summary>
        private const int MIN_RR_FOR_DFA_ALPHA1 = DFA_N_MAX_SHORT * 4;

        /// <summary>
        /// 计算 DFA α2 所需的最少 RR 间期数量。
        /// 窗口 16-32，至少 4 个窗口，约需 128 个 RR 间期（约 2 分钟）。
        /// </summary>
        ...
        /// 2. 将轮廓等分为大小为 n 的窗口，对每个窗口做线性拟合
        /// 3. 计算去趋势后的均方根波动 F(n)
        /// 4. 在双对数坐标 log(n) vs log(F(n)) 上做线性回归
        /// 5. 回归斜率即为标度指数 α
        ///
        /// 参考：
        ...
        private static double ComputeDfaAlpha(List<double> rr, int nMin, int nMax)
        {
            ...
            // 步骤 2 & 3：对每个窗口大小 n，计算波动函数 F(n)
            // 收集 (log(n), log(F(n))) 数据点用于回归
            ...
            {
                int numWindows = N / n;
                if (numWindows < 4) break; // 至少需要 4 个窗口
                ...
                // 反向窗口（从末尾开始，利用剩余数据）
                if (N - numWindows * n >= n / 2)
                {
                    int reverseStart = N - n;
                    sumSqResidual += ComputeWindowResidualSumSq(y, reverseStart, n);
                    totalPoints += n;
                }
